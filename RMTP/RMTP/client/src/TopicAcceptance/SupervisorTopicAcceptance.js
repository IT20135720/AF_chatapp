...
export default class SupervisorTopicAcceptance extends Component {
    constructor(props) {
        super(props);
    ...
    onDelete = (id) => {
        if (window.confirm('Are You Sure?')) {
            ...
            axios.delete(`/topicacceptance/delete/${id}`).then((res) => {
                    ...
                    <tbody>
                        {this.state.posts.map((posts) => (
                            <tr key={posts._id}>
                                ...
                                <td>{posts.status}</td>
                                <td>
                                    <a className="btn btn-warning text-dark" href={`/editstatus/${posts._id}`}>
